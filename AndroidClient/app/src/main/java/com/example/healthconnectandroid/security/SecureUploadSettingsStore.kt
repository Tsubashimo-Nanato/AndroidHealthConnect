...
internal object SecureUploadSettingsStore {
    ...
    private const val AUTHENTICATED_DATA = "com.example.healthconnectandroid.upload-settings.v1"
    ...
    fun read(
    ...
    ): SecureUploadSettingsReadResult {
        val appContext = context.applicationContext
        val preferences = try {
            securePreferences(appContext, storageScope)
        } catch (_: Exception) {
        ...
        val cleartext = try {
            ...
            cipher.updateAAD(additionalAuthenticatedData(storageScope))
    ...
    fun write(
        ...
        val appContext = context.applicationContext
        return try {
            if (replaceCorruptKey && !deleteExistingKey(storageScope)) return false
            val cipher = Cipher.getInstance(CIPHER_TRANSFORMATION)
            cipher.init(Cipher.ENCRYPT_MODE, getOrCreateKey(storageScope))
            cipher.updateAAD(additionalAuthenticatedData(storageScope))
            val ciphertext = cipher.doFinal(UploadSettingsPayloadCodec.encode(settings))

            securePreferences(appContext, storageScope).edit()
    ...
    private fun getOrCreateKey(storageScope: String?): SecretKey {
        existingKey(storageScope)?.let { return it }
        val generator = KeyGenerator.getInstance(KeyProperties.KEY_ALGORITHM_AES, ANDROID_KEY_STORE)
        val specification = KeyGenParameterSpec.Builder(
            scopedValue(KEY_ALIAS, storageScope),
    ...
    private fun existingKey(storageScope: String?): SecretKey? {
        val keyStore = KeyStore.getInstance(ANDROID_KEY_STORE).apply { load(null) }
        return keyStore.getKey(scopedValue(KEY_ALIAS, storageScope), null) as? SecretKey
    }
    ...
    private fun deleteExistingKey(storageScope: String?): Boolean =
        try {
            val keyStore = KeyStore.getInstance(ANDROID_KEY_STORE).apply { load(null) }
            val keyAlias = scopedValue(KEY_ALIAS, storageScope)
    ...
    private fun additionalAuthenticatedData(storageScope: String?): ByteArray =
        scopedValue(AUTHENTICATED_DATA, storageScope).toByteArray(StandardCharsets.UTF_8)

    private fun securePreferences(context: Context, storageScope: String?) =
        context.getSharedPreferences(scopedValue(PREFS_NAME, storageScope), Context.MODE_PRIVATE)

    private fun scopedValue(base: String, storageScope: String?): String {
        if (storageScope == null) return base
        require(profileIdPattern.matches(storageScope)) { "Invalid upload settings storage scope" }
