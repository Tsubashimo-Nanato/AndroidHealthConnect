...
private val exactChinese = mapOf(
    ...
    "Local health data viewer, CSV exporter, and sync tool." to "本地健康数据查看、CSV 导出和同步工具。",
    ...
    "Health Connect read permissions are used only for this app's local data flow" to "Health Connect 读取权限仅用于此应用的本地数据流程",
    ...
    "Data is stored locally in this app's database for inspection, charting, CSV export, and manual or periodic sync actions. The app does not write Health Connect data." to "数据会存储在此应用的本地数据库中，用于查看、绘图、CSV 导出以及手动或周期同步。应用不会写入 Health Connect 数据。",
    ...
    "Yes" to "是",
    "No" to "否",
    "Heart-Rate Compatibility Tools" to "心率兼容工具",
